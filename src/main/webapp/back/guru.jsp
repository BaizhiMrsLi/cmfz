...
<script>
    $(function () {
        ...
        $('#gurulist').jqGrid({
            ...
            editurl: "${pageContext.request.contextPath}/guru/opt",//开启编辑时执行编辑操作的url路径  添加  修改  删除
            ...
            {},//开启编辑操作，就是增删改查的小工具，是否开启哪一个(默认值为true)
            {
                ...
                afterSubmit: function (response,postData) {
                    ...
                    if (code == "updateOk") {
                        $.ajaxFileUpload({
                            url: "${pageContext.request.contextPath}/guru/motifyUpload",//用于文件上传的服务器端请求地址
            ...
            },
            {
                ...
                afterSubmit: function (response,postData) {
                    ...
                    if (code == "addOk") {
                        $.ajaxFileUpload({
                            url: "${pageContext.request.contextPath}/guru/upload",//用于文件上传的服务器端请求地址
                            fileElementId: 'aa',    //文件上传空间的id属性  <input type="file" id="file" name="file" />
                            //dataType: 'json',       //返回值类型 一般设置为json
                            //type:'POST',
                            data: {id: id},
